...
These tags are used by the event store (UmaDB) for DCB queries. When a command requests events with `user_id=42`, only events tagged `user_id:42` are returned. (As in [Events](./events.md#domain-ids), the tag name is the field name as written: `user_id` in Rust, `userId` in TypeScript.)
...
See [Events → Scoping which events a fold sees](./events.md#scoping-which-events-a-fold-sees) for the full description and examples.
