...
pub fn upload(
    ...
    let Some((idempotent, response)) =
        client.upload_wasm("projectors", &name, &version, &env_vars, &file, activate)?
    else {
...
pub fn delete(client: &ApiClient, name: String, version: Option<String>, yes: bool) -> Result<()> {
    ...
    if !yes && !confirm_destructive(&format!("delete projector '{name}' and all its versions?"))? {
        println!("aborted");
        return Ok(());
    }
    ...
    if response.deleted {
        println!(
            "{} deleted projector {name} ({} versions)",
            "✓".green(),
            response.versions_deleted
        );
    } else {
        println!("{} projector {name} not found", "!".yellow());
    }
...
pub fn replay(client: &ApiClient, name: String) -> Result<()> {
    let path = format!("/projectors/{name}/replay");
