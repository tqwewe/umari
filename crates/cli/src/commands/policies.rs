...
pub fn upload(
    ...
    let (idempotent, response) = client.upload_wasm("policies", &name, &version, &file, activate)?;
...
pub fn activate(client: &ApiClient, name: String, version: String) -> Result<()> {
    let path = format!("/policies/{name}/active");
...
pub fn replay(client: &ApiClient, name: String) -> Result<()> {
    let path = format!("/policies/{name}/replay");
