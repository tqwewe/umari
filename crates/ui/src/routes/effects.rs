...
use crate::{
    UiState,
    components::{
        ModuleHealth, default_sql_query, delete_module_button, env_vars_panel, module_status_card,
        module_summary_table, output, run_sql_query, sql_query_section, tabs, upload_form,
        versions_table,
    },
    error::HtmlError,
    htmx::respond,
...
pub async fn get_effect(
    ...
    let env_panel = env_vars_panel(ModuleType::Effect, &name, &env_vars);
    ...
    let content = html! {
        ...
        div class="flex items-start justify-between gap-4 mb-6" {
            h2 class="text-2xl font-semibold text-gray-900 dark:text-gray-100" { "Effect: " (name) }
            div class="flex flex-col items-end gap-1" {
                (delete_module_button(ModuleType::Effect, &name))
            }
        }
