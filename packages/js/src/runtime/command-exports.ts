...
export interface CommandExports {
  ...
  execute(input: string, context: WitCommandContext): WitExecuteOutput;
}
...
export function exportCommand<TInput extends object, TFolds extends FoldsMap>(
  ...
  return {
    ...
    execute(rawInput, witCtx): WitExecuteOutput {
      let input: TInput;
      try {
      ...
      try {
        ...
        if (err instanceof RejectError)
          throw { tag: "rejected", val: err.message } as WitCommandError;
        if (err instanceof InvalidInputError)
          throw { tag: "invalid-input", val: err.message } as WitCommandError;
...
function runCommand<TInput extends object, TFolds extends FoldsMap>(
  ...
  const folds = def.folds(input) as Readonly<
    Record<string, BoundFold<unknown>>
  >;
  ...
  const query = buildDcbQuery(
    entries,
    entries.length === 0 ? [] : [cmdBindings],
  );
...
function extractFoldStates<TFolds extends FoldsMap>(
  states: FoldStateMap,
): FoldStates<TFolds> {
