...
| [`umari-server`](crates/server) | Server binary: runtime, API, and web UI |
...
Contributions are welcome. [Open an issue](https://github.com/tqwewe/umari/issues), improve the book in [`docs/src`](docs/src), or pick up existing work. To build the book locally, run `cargo install mdbook mdbook-tabs`, then `mdbook serve docs` (the tab theme assets are checked in under `docs/theme/`).
